...
Telegram is the current production channel adapter. `ChannelPort` and
`CompositeChannelMux` provide extension seams for additional channels; those
channels are not current capabilities until a concrete adapter is implemented and
wired.
...
```mermaid
flowchart LR
    U[Telegram user/group/topic] --> T[Telegram adapter]
    T --> D[Debouncer]
    D --> E[Core engine]
